  	$(document).ready(function() { 
		...
		var check_permission = jQuery.inArray( "notice_award_view_all", check_user_access );
		...
		// GET Notice of Award
		jQuery.ajax({
		url: baseUrl +project_id+"/notice-award",
		    type: "GET",
		    headers: {
		...
		.done(function(data, textStatus, jqXHR) {
		    console.log(data.data);
			    ...
			    var specific_project_name = 'Notice of Award for Project: ' + window.project_name;
			...
			jQuery.each( data.data, function( i, val ) {
				...
				if(check_permission < 1){
					...
					var update_permission = '<a href="'+baseUrl+'dashboard/'+val.p_id+'/notice_award/'+val.pna_id+'/update" class="btn btn-primary btn-xs tooltips hide_update_permission" data-placement="top" data-toggle="tooltip" data-original-title="Edit"><i class="fa fa-pencil"></i></a>';
				}

				var status = val.pna_status;
				if(status == 'active'){
