  	$(document).ready(function() { 
		...
		.fail(function(jqXHR, textStatus, errorThrown) {
     	...
     	// Get login user profile data
     	$("#update_specifications_form").hide();
		var token = localStorage.getItem('u_token');
		...
		url: baseUrl + "standards/"+standard_id,
		    ...
		    $("#update_specifications_form").show();
		    $(".loading_data").hide();
		})
		.fail(function(jqXHR, textStatus, errorThrown) {
		    console.log("HTTP Request Failed");
    ...
    $('#update_specifications_form').submit(function(e) {
        e.preventDefault();
        $('.loading-submit').show();
        ...
        .done(function(data, textStatus, jqXHR) {
            ...
            html = '<div id="toast-container" class="toast-top-right" aria-live="polite" role="alert" style="margin-top:50px;"><div class="toast toast-success">Specification updated successfully!</div></div>';
            $("#alert_message").html(html);
            setTimeout(function()
                {
                    $("#alert_message").hide();
