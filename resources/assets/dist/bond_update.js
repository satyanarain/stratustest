  	$(document).ready(function() {
	   ...
	   // Get login user profile data
		...
		bond_id = url[ url.length - 2 ]; // projects
		console.log(bond_id);
    ...
    $('#update_bond_form').submit(function(e) {
      $('.loading-submit').show();
        ...
        var token = localStorage.getItem('u_token');
        jQuery.ajax({
            url: baseUrl + "bond/"+bond_id+"/update",
            type: "POST",
            data: {
         	    "status"            : status,
            ...
            // $("#alert_message").html(html);
                $("#alert_message").show();
                $('.loading-submit').hide();
                html = '<div id="toast-container" class="toast-top-right" aria-live="polite" role="alert" style="margin-top:50px;"><div class="toast toast-success">Bond updated successfully!</div></div>';
                $("#alert_message").html(html);
                setTimeout(function()
                {
                    $("#alert_message").hide();
