  	$(document).ready(function() { 
		...
		var check_permission = jQuery.inArray( "plan_view_all", check_user_access );
		...
		jQuery.ajax({
		url: baseUrl +project_id+"/project-plan",
		    type: "GET",
		    headers: {
		    ...
		    // console.log(data.data);
			    ...
			    var specific_project_name = 'Plans for Project: ' + window.project_name;
			...
			jQuery.each( data.data, function( i, val ) {
				...
				var check_permission = jQuery.inArray( "plan_update", check_user_access );
				console.log(check_permission);
				if(check_permission < 1){
					var update_permission = '';
				}
				else {
					var update_permission = '<a href="'+baseUrl+'dashboard/'+val.p_id+'/plans/'+val.pp_id+'/update" class="btn btn-primary btn-xs tooltips hide_update_permission" data-placement="top" data-toggle="tooltip" data-original-title="Edit"><i class="fa fa-pencil"></i></a>';
				}

				var status = val.pp_status;
				if(status == 'active'){
		    	status = '<span class="label label-success">Active</span>';
			  	...
			  	if(doc_path == null){
			  		...
			  		if(val.pp_status == 'active'){
			  			doc_path_value = '<td><a href="'+baseUrl+val.doc_path+'" target="_blank"><img src="'+baseUrl+'resources/assets/img/pdf_icon.png" width="40"/></a></td>';
			  		}
			  		else {
			  	...
			  	if(val.pp_approval == 'yes'){
			  		var val_approval = 'Yes';
			  	}
			  	else {
			  		var val_approval = 'No';
			  	}
     ...
     //                '<a href="'+baseUrl+'dashboard/'+val.p_id+'/plans/'+val.pp_id+'/update" class="btn btn-primary btn-xs tooltips" data-placement="top" data-toggle="tooltip" data-original-title="Edit"><i class="fa fa-pencil"></i></a>'+
     ...
     //            '</td></tr>'
			  ...
			  var t = $('#view_users_table').DataTable();
				t.row.add( [
		           count,
		           val.pp_name,
                   // $.datepicker.formatDate('yy-mmdd', new Date(val.pp_date.replace(' ', 'T'))),
                   val.pp_date,
		           val_approval,
		           doc_path_value,
		           status,
