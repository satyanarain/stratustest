  	$(document).ready(function() {
		...
		contract_item_id = url[ url.length - 2 ]; // projects
		...
		url: baseUrl + "bid-items/"+contract_item_id,
		    ...
		    .done(function(data, textStatus, jqXHR) {
		    console.log(data);
		    var item_description = data.data.pbi_item_description;
		    $('#item_description').val(item_description);
		    var item_qty = data.data.pbi_item_qty;
		    $('#item_qty').val(item_qty);
		    var item_unit_price = data.data.pbi_item_unit_price;
            $('#item_unit_price').val(item_unit_price);

            var pbi_item_unit_other = data.data.pbi_item_unit_other;
		    $('#item_unit_other').val(pbi_item_unit_other);


		    var status = data.data.pbi_status;
		    if(status == "active"){
		    	status = 'active';
		    }
		    else {
		    ...
		    $('#status').val(status);
		    $('#item_unit').val(data.data.pbi_item_unit);
            $("#update_standards_form").show();
            if(data.data.pbi_item_unit == 'other' || data.data.pbi_item_unit == 'Other'){
		      $(".other_option").show();  
            }
		    $(".loading_data").hide();
		})
    ...
    $('#update_contract_item_form').submit(function(e) {
      $('.loading-submit').show();
        ...
        var token = localStorage.getItem('u_token');
        jQuery.ajax({
            url: baseUrl + "bid-items/"+contract_item_id+"/update",
            type: "POST",
            data: {
            ...
            // window.location.href = baseUrl + "dashboard/"+project_id+"/contract_item";
                $('html, body').animate({
