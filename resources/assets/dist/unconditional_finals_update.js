$(document).ready(function() {
    ...
    puf_id = url[ url.length - 2 ]; // projects
    ...
    jQuery.ajax({
        ...
        .done(function(data, textStatus, jqXHR) {
            console.log(data);
            var status = data.data.puf_status;
            if(status == "active"){
                status = 'active';
            }
            else {
...
$('#update_swppp_form').submit(function(e) {
  $('.loading-submit').show();
    ...
    var token = localStorage.getItem('u_token');
    jQuery.ajax({
        url: baseUrl + "unconditional_finals/"+puf_id+"/update",
        type: "POST",
        data: {
            // "date"              : date_of_report,
            // "name"              : name_of_report,
        ...
        .done(function(data, textStatus, jqXHR) {
            ...
            $('.loading-submit').hide();
            html = '<div id="toast-container" class="toast-top-right" aria-live="polite" role="alert" style="margin-top:50px;"><div class="toast toast-success">Unconditional finals updated successfully!</div></div>';
