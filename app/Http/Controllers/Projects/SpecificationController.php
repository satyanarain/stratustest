    ...
    public function add_standards(Request $request)
    {
        try
        {
          ...
          // else { 
            ...
            ->where('pup_permission_key', '=', 'specification_add')
            ->get();
            ...
            else
            {
                ...
                ->insert(['ps_agency_name' => $agency_name, 'ps_name' => $name, 'ps_date' => $date, 'ps_url' => $url, 'ps_applicable' => $applicable, 'ps_type' => $type, 'ps_file_path' => $upload_file, 'ps_project_id' => $project_id, 'ps_user_parent' => $user_id, 'ps_status' => $status]);
                ...
                else
                {
                  ...
                  foreach ($check_project_users as $check_project_user) {
                    ...
                    $project_id           = $project_id;
                    $notification_title   = 'Add new Specification in Project: ' .$check_project_user->p_name;
                    $url                  = App::make('url')->to('/');
                    $link                 = "/dashboard/".$project_id."/specifications";
                    $date                 = date("M d, Y h:i a");

                    if($applicable == 'no'){
                      $email_description    = 'New specification added in Project: <strong>'.$check_project_user->p_name.'</strong> that specification is not applicable yet <a href="'.$url.$link.'"> Click Here to see </a>';
                    }
                    else {
                      $email_description    = 'New specification added in Project: <strong>'.$check_project_user->p_name.'</strong> <a href="'.$url.$link.'"> Click Here to see </a>';
                    }
                  ...
                  $result = array('description'=>"Add standard or specification successfully",'code'=>200);
    ...
    */
    public function update_standards(Request $request, $spec_id)
    {
        try
        {
          ...
          ->where('pup_project_id', '=', $project_id)
          ->where('pup_user_id', '=', $user_id_permission)
          ->where('pup_permission_key', '=', 'specification_update')
          ->get();
          if(count($permission_query) < 1)
          {
            $result = array('code'=>403, "description"=>"Access Denies");
          ...
          else
          {
            ...
            else
            {
              ...
              ->where('ps_id', '=', $spec_id)
              ->first();
// if(($agency_name == "") ? $agency_name = $detail->ps_agency_name : $agency_name = $agency_name);
// if(($name == "") ? $name = $detail->ps_name : $name = $name);
// if(($date == "") ? $date = $detail->ps_date : $date = $date);
// if(($url == "") ? $url = $detail->ps_url : $url = $url);
// if(($applicable == "") ? $applicable = $detail->ps_applicable : $applicable = $applicable);
// if(($upload_file == "") ? $upload_file = $detail->ps_file_path : $upload_file = $upload_file);

                $query = DB::table('project_standards')
                ->where('ps_id', '=', $spec_id)
                // ->update(['ps_agency_name' => $agency_name, 'ps_name' => $name, 'ps_date' => $date, 'ps_url' => $url, 'ps_applicable' => $applicable, 'ps_type' => $type, 'ps_file_path' => $upload_file, 'ps_project_id' => $project_id, 'ps_user_parent' => $user_id, 'ps_status' => $status]);
                ...
                else
                {
                  ...
                  foreach ($check_project_users as $check_project_user) {
                    ...
                    // Notification Parameter
                    $project_id           = $project_id;
                    $notification_title   = 'Change status specification # '.$spec_id.' in Project: ' .$check_project_user->p_name;
                    $url                  = App::make('url')->to('/');
                    $link                 = "/dashboard/".$project_id."/specifications";
                    $date                 = date("M d, Y h:i a");
                    $email_description    = 'Change status specification # '.$spec_id.' in Project: <strong>'.$check_project_user->p_name.'</strong> <a href="'.$url.$link.'"> Click Here to see </a>';
    ...
    */
    public function get_standards_project(Request $request, $project_id, $spec_type)
    {
      try
      {
        ...
        // Start Permission Process
          ...
          ->where('pup_project_id', '=', $project_id)
          ->where('pup_user_id', '=', $user_id_permission)
          ->where('pup_permission_key', '=', 'specification_view_all')
          ->get();
          if(count($permission_query) < 1)
          {
            $result = array('code'=>403, "description"=>"Access Denies");
    ...
    // $document_path = $request['test'];
    // // echo '<pre>';
    // // print_r($faizan);
    // // echo '</pre>';
    //   if (Input::file('file')->isValid()) {
    //     $destinationPath = $document_path; // upload path
    //     // $destinationPath = '/uploads/standard/'; // upload path
