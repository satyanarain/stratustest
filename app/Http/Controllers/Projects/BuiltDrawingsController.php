      ...
      return response()->json($result, 403);
        ...
        else
        {
            ...
            else
            {
              ...
              $project_id           = $project_id;
              $notification_title   = 'Add new built drawing in Project: ' .$check_project_user->p_name;
              $url                  = App::make('url')->to('/');
              $link                 = "dashboard/".$project_id."/built_drawing/".$query;
              $date                 = date("M d, Y h:i a");
              $email_description    = 'Add new built drawing in Project: <strong>'.$check_project_user->p_name.'</strong> <a href="'.$url.$link.'"> Click Here to see </a>';
