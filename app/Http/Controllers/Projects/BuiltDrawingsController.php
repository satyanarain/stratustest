      ...
      return response()->json($result, 403);
        ...
        else
        {
            ...
            else
            {
              ...
              $project_id           = $project_id;
              $notification_title   = 'New built drawing added in Project: ' .$check_project_user->p_name;
              $url                  = App::make('url')->to('/');
              $link                 = "dashboard/".$project_id."/built_drawing/".$query;
              $date                 = date("M d, Y h:i a");
              $email_description    = 'New built drawing added in Project: <strong>'.$check_project_user->p_name.'</strong> <a href="'.$url.$link.'"> Click Here to see </a>';
    ...
    try
    {
      ...
      // else {
        ...
        if ($validator->fails()) {
          ...
          $query = DB::table('project_built_drawing')
          ->where('pbd_id', '=', $pbd_id)
          ->update(['pbd_engineer_redline' => $engineer_redline, 'pbd_user_id' => $user_id, 'pbd_change_plan' => $built_plan, 'pbd_status' => $status]);
          if(count($query) < 1)
          {
            $result = array('code'=>400, "description"=>"No records found");
            return response()->json($result, 400);
