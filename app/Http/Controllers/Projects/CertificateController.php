    ...
    if(count($check_single_user_permission) < 1){
      ...
      return response()->json($result, 403);
        ...
        else
        {
            ...
            ->update([ 'ci_liability_currency' => $liability_currency, 'ci_liability_limit' => $liability_limit, 'ci_liability_exp' => $liability_exp, 'ci_liability_required_min' => $liability_required_min, 'ci_liability_cert_path' => $liability_cert_path, 'ci_work_comp_limit' => $work_comp_limit, 'ci_work_comp_exp' => $work_comp_exp, 'ci_works_comp_include_above' => $works_comp_include_above, 'ci_works_comp_required_min' => $works_comp_required_min, 'ci_works_comp_not_include' => $works_comp_not_include, 'ci_work_comp_cert_path' => $work_comp_cert_path,  'ci_auto_liability_limit' => $auto_liability_limit,  'ci_auto_liability_exp' => $auto_liability_exp, 'ci_auto_include_above' => $auto_include_above, 'ci_auto_liability_required_min' => $auto_liability_required_min, 'ci_auto_liability_not_include' => $auto_liability_not_include, 'ci_auto_liability_cert_path' => $auto_liability_cert_path,  'ci_umbrella_liability_limit' => $umbrella_limit,  'ci_umbrella_liability_exp' => $umbrella_exp,'ci_umbrella_liability_cert_path' => $umbrella_cert_path, 'ci_doc_id_certificate' => $upload_doc_id_certificate, 'ci_project_id' => $project_id, 'ci_user_id' => $user_id, 'ci_status' => $status]);
            if(count($query) < 1)
            {
              $result = array('code'=>400, "description"=>"No records found");
              return response()->json($result, 400);
